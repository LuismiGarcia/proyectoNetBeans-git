...
class Plantilla
{
    ...
    public static function mostrarFormularioJugada(): string
    {
        ...
        for ($n = 0; $n < 4; $n++) {
            $selectores .= "\n<select onclick='cambia_color($n)' name='combinacion$n' id='combinacion$n'>\n";
    ...
    public static function mostrarTodasJugadas(): string
    {
        ...
        foreach ($jugadas as $numJugada => $jugada) {
            ...
            $numeroDeJugadaAmostrar = count($jugadas)-$contador;
            $msj .= "<br /><br /><h2>Valor de la jugada $numeroDeJugadaAmostrar :</h2><br /> $colores<br />";
            $contador++;
        }
        return $msj;
    ...
    private static function obtenerDescripcionJugadas(array $jugadas): string
    {
        ...
        foreach ($jugadas as $numeroJugada => $jugada) {
            $jugada = Jugada::objeto($jugada);
            $numeroJugadaAmostrar = count($jugadas)-$contador;
            $htmlJugadas .= "<div class='jugada'>";
            $htmlJugadas .= "<h3 class=titulo jugadaNumero>Jugada" . $numeroJugadaAmostrar .  "</h3>\n";
            $contador++;
